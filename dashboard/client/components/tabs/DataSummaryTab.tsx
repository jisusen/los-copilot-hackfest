...
function Row({ label, testId, value }: { label: string; testId: string; value: React.ReactNode }) {
  return (
    <div
      ...
      className="flex flex-col sm:flex-row sm:items-center py-3 px-4 gap-0.5 sm:gap-0"
      style={{ borderBottom: '1px solid #f1f5f9' }}
    >
      <span className="w-full sm:w-56 flex-shrink-0 text-xs sm:text-sm font-medium field-label" style={{ color: '#94a3b8' }}>{label}</span>
      <span
    ...
    <div data-testid="tab-content-data-summary" className="pb-8">
      ...
      {crde && (
        <Section title="CRDE Result">
          ...
          {rules.length > 0 && (
            <div className="flex flex-col sm:flex-row py-3 px-4 gap-0.5 sm:gap-0" style={{ borderBottom: '1px solid #f1f5f9' }}>
              <span className="w-full sm:w-56 flex-shrink-0 text-xs sm:text-sm font-medium field-label" style={{ color: '#475569' }}>Triggered Rules</span>
