...
function reducer(state: SessionsMap, action: Action): SessionsMap {
  ...
  switch (action.type) {
    case 'PROGRESS': {
      ...
      const logs = existing?.status === 'running' ? [...existing.logs, action.step] : [action.step];
      next.set(action.appId, {
        status: 'running',
        logs,
        pct: action.pct,
        elapsedMs: action.elapsedMs,
        startedAt: existing?.status === 'running' ? existing.startedAt : Date.now(),
      });
      break;
    }
    case 'COMPLETE': {
    ...
    case 'ERROR': {
      next.set(action.appId, { status: 'error', error: action.error, retryable: action.retryable });
      break;
    }
    case 'DECIDED': {
      const existing = state.get(action.appId);
...
export function useAgentSessions() {
  const [sessions, dispatch] = useReducer(reducer, new Map<string, AgentState>());

  const handleWsMessage = useCallback((msg: WsMessage) => {
    if (msg.type === 'agent:progress') {
      dispatch({ type: 'PROGRESS', appId: msg.appId, step: msg.step, pct: msg.pct, elapsedMs: msg.elapsedMs });
    } else if (msg.type === 'agent:complete') {
      dispatch({ type: 'COMPLETE', appId: msg.appId, result: { status: 'ready', result: msg.result, elapsedMs: msg.elapsedMs }, elapsedMs: msg.elapsedMs });
