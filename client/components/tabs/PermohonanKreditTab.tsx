...
export function PermohonanKreditTab({
  application,
  financials,
}: {
  ...
  return (
    <div data-testid="tab-content-permohonan-kredit">
      ...
      >
        ...
        <Field label="Tenor" testId="tenor">
          {application.tenor_months} months
        </Field>
        <Field label="Interest Rate" testId="suku-bunga">
          {application.interest_rate}% p.a. (effective)
        </Field>
        ...
        <Field label="Monthly Installment" testId="cicilan">
          <span className="font-mono">
            {formatRp(financials.requested_installment)}
          </span>
        </Field>
        <Field label="Loan Purpose" testId="tujuan">
          {application.loan_purpose}
        </Field>
        <Field label="Referral Source" testId="referral">
          {application.referral_source}
        </Field>
        <Field label="Payment Source" testId="sumber-bayar">
      ...
      <div
        ...
        style={{ borderColor: "#e2e8f0" }}
      >
        <Field label="Application Date" testId="tanggal-permohonan">
          {formatDate(application.created_at)}
        </Field>
        <Field label="Branch" testId="cabang">
          {application.branch}
        </Field>
        <Field label="Marketing Officer" testId="marketing-officer">
          {application.marketing_officer}
        </Field>
        {application.legal_docs_complete && (
          <Field label="Legal Documents" testId="dokumen-hukum">
            <span
              ...
              style={{
                background:
                  application.legal_docs_complete === "Complete"
                    ? "#e3f0e9"
                    : "#fff1d8",
                color:
                  application.legal_docs_complete === "Complete"
      ...
      {application.disbursement_date && (
        <>
          ...
          <div
            ...
            style={{ borderColor: "#e2e8f0" }}
          >
            <Field label="Target Disbursement" testId="tanggal-cair">
              {application.disbursement_date}
            </Field>
            <Field label="Disbursement Method" testId="metode-cair">
            ...
            <Field label="Repayment Method" testId="metode-bayar">
          ...
          <div
            ...
            style={{ borderColor: "#e2e8f0" }}
          >
            <Field label="Provision Fee" testId="provisi">
              {application.provisi_fee}
            </Field>
            <Field label="Administration Fee" testId="biaya-admin">
              <span className="font-mono">
                {formatRp(application.admin_fee)}
              </span>
            </Field>
            <Field label="Insurance Required" testId="asuransi">
              {application.insurance_required}
            </Field>
            {application.insurance_required === "Yes" && (
              <Field label="Insurance Type" testId="jenis-asuransi">
